...
class Network:
    ...
    def __init__(self, layer_counts):
        ...
        #self.biases = np.array([.5]*self.depth, dtype=float)
    ...
    def train(self, t_d, truth_vector=None):
        ...
        for x in range(0, max_tests):
            ...
            if x % (1000) == 0:
                ...
                if previous_error - current_error < 0.0001:
                    break
                    #pass
                
                previous_error = current_error
                
                # print("{}, expected {}, err {:.3f}".format(prop,
                #                                            str(expected[ran]),
                #                                            current_error))
    ...
    def test(self, training_data, tests):
        ...
        for x in range(0, tests):
            ...
            expected = classifiers[pair[1]].index(1)
...
def main():
    ...
    #t_d = data_from_csv("wdbc.csv", 1, [x for x in range(12,32)] + [0])
    t_d.normalize()
    ...
    nn.test(test_data, 10)

    #print(nn.enum_arcs())

    # t_d = data_from_csv("MNIST_train.csv", 0)
    # t_d.normalize()
    # #import pdb; pdb.set_trace()
    # nn = Network([784, 40, 10])
    # nn.train(t_d)

    # tst_d = data_from_csv("MNIST_test.csv", 0)
    # tst_d.normalize()
    # nn.test(tst_d, 20)
    ...
    #     nn.backprop(t_d[0][0], np.array([0, 1]))
    #     if x % 10 == 0:
    #         print(nn.backprop(t_d[0][0], np.array([0, 1])))
